...
const uploadStudents = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  console.log(
    students[0]["Courses"].split(";").map((c) => c.trim().toUpperCase())
  );
  const studentsToInsert = students.map((d) => {
    return {
      fullName: toTitleCase(d["Full Name"].trim()),
      jamb: d["Matric number/jamb reg. number"].toUpperCase(),
      department: toTitleCase(d["Department"].trim()),
      courseSelections: d["Courses"]
        .split(";")
        .map((c) => c.trim().toUpperCase()).join(";"),
      phoneNumber:
